...
namespace Exploder
{
    ...
    private float isolatingIslandsTime = 0.0f;
        ...
        private void Awake()
        {
            ...
            // init pool
            FragmentPool.Instance.Allocate(FragmentPoolSize, MeshColliders, use2d, FragmentPrefab);
        ...
        private bool IsInRadius(GameObject o)
        {
            ...
            if (UseCubeRadius)
            {
                ...
                var localC = transform.InverseTransformPoint(mainCentroid);

                return (Mathf.Abs(localP.x - localC.x) < CubeRadius.x &&
                        Mathf.Abs(localP.y - localC.y) < CubeRadius.y &&
				        Mathf.Abs(localP.z - localC.z) < CubeRadius.z);
            }
            else
            {
                return Radius*Radius > (centroid - mainCentroid).sqrMagnitude;
            }
        }
...
//            }
        ...
        void InitPostprocess()
        {
            ...
            FragmentPool.Instance.Allocate(fragmentsNum, MeshColliders, use2d, FragmentPrefab);
            FragmentPool.Instance.SetDeactivateOptions(DeactivateOptions, FadeoutOptions, DeactivateTimeout);
            FragmentPool.Instance.SetExplodableFragments(ExplodeFragments, DontUseTag);
            FragmentPool.Instance.SetFragmentPhysicsOptions(FragmentOptions, use2d);
            FragmentPool.Instance.SetSFXOptions(SFXOptions);
